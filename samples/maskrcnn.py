...
def display_instances(image,boxes,masks,ids,names,scores):
   ...
   # rospy.init_node("beef_detector_node")
    ...
    global resultPressPose_one
    global resultPressPose_two
    global resultPressPose_three
    global resultPressPose_four
    ...
    for i,color in enumerate(colors):
        ...
        image=cv2.putText(
            image,caption,(x1,y1),cv2.FONT_HERSHEY_COMPLEX,0.5,(255,0,0),2
        )
    
    #image_pub = rospy.Publisher("/detector/image_result",Image,queue_size=10)
...
class_names = ['BG', 'person', 'bicycle', 'car', 'motorcycle', 'airplane',
            ...
            'kite', 'baseball bat', '0 glove', 'skateboard',
...
def model_detection(img):
    ...
    center_pose_pub = rospy.Publisher("/maskrcnn/centerpose",Float32MultiArray,queue_size=1)
    grasp_pose_left_pub = rospy.Publisher("/maskrcnn/grasp_pose_left",Float32MultiArray,queue_size=1)
    grasp_pose_right_pub = rospy.Publisher("/maskrcnn/grasp_pose_right",Float32MultiArray,queue_size=1)
    grasp_theta_left_right_pub = rospy.Publisher("/maskrcnn/grasp_theta_left_right",Float32MultiArray,queue_size=1)
    press_pose_one_pub = rospy.Publisher("/maskrcnn/press_pose_one",Float32MultiArray,queue_size=1)
    press_pose_two_pub = rospy.Publisher("/maskrcnn/press_pose_two",Float32MultiArray,queue_size=1)
    press_pose_three_pub = rospy.Publisher("/maskrcnn/press_pose_three",Float32MultiArray,queue_size=1)
    press_pose_four_pub = rospy.Publisher("/maskrcnn/press_pose_four",Float32MultiArray,queue_size=1)
